...
def page_extract(date):
    ...
    try:
        ...
        boxoffice_collection_daily={
            'url':link,
            'extracted_at':str(datetime.now().date()),
        ...
        s3.put_object(Bucket=bucket,Key=path,Body=json.dumps(boxoffice_collection_daily,indent=2))
...
def movie_page_extract(collection_parsed_fp,run_date:str): 
    ...
    s3_data=s3.get_object(Bucket=collection_parsed_fp.split('/')[2],Key='/'.join(collection_parsed_fp.split('/')[3:]))['Body'].read().decode('utf-8')
    collection_html_source=json.loads(s3_data)
    try:
        ...
        s3.put_object(Bucket=bucket,Key=path,Body=json.dumps(movies_parsed,indent=2))
...
def movie_cast_filmmakers_extract(parsed_movie_json_fp,run_date:str): # Takes the file path to individual movie and extracts and save the cast and crew info page from imdb.com to cast_pages directory
    driver=webdriver.Chrome(options=options)
    s3_data=s3.get_object(Bucket=parsed_movie_json_fp.split('/')[2],Key='/'.join(parsed_movie_json_fp.split('/')[3:]))['Body'].read().decode('utf-8')
    movies_parsed=json.loads(s3_data)
...
def distributor_scraper(movie_parsed_fp,run_date:str):
    ...
    '''
    s3_data=s3.get_object(Bucket=movie_parsed_fp.split('/')[2],Key='/'.join(movie_parsed_fp.split('/')[3:]))['Body'].read().decode('utf-8')
    movies_parsed=json.loads(s3_data)
