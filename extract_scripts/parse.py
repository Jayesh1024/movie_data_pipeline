...
def fact_parser(collection_json_path,run_date:str):
    s3_data=s3.get_object(Bucket=collection_json_path.split('/')[2],Key='/'.join(collection_json_path.split('/')[3:]))['Body'].read().decode('utf-8')
    collection_json=json.loads(s3_data)
    ...
    if movie_rows:
        ...
        logger.warning("movie_rows is NONE, collection_parsed will be empty")    
        
    path=f'run_{run_date}/collection_parsed_{run_date}.json'
    s3.put_object(Bucket=bucket,Key=path,Body=json.dumps(movie_fact_table,indent=2))
...
def movie_parser(movies_json_fp,run_date:str):
    ...
    s3_data=s3.get_object(Bucket=movies_json_fp.split('/')[2],Key='/'.join(movies_json_fp.split('/')[3:]))['Body'].read().decode('utf-8')
    movies_json=json.loads(s3_data)
    ...
    s3.put_object(Bucket=bucket,Key=path,Body=json.dumps(movies,indent=2))

    return f"s3://{bucket}/{path}"

def genre_parser(genre_html_json_fp,run_date:str):
    
    s3_data=s3.get_object(Bucket=genre_html_json_fp.split('/')[2],Key='/'.join(genre_html_json_fp.split('/')[3:]))['Body'].read().decode('utf-8')
    genre_html=json.loads(s3_data)
    ...
    s3.put_object(Bucket=bucket,Key=path,Body=json.dumps(response,indent=2))
...
def filmmaker_parser(filmmaker_html_fp,run_date:str):
    ...
    s3_data=s3.get_object(Bucket=filmmaker_html_fp.split('/')[2],Key='/'.join(filmmaker_html_fp.split('/')[3:]))['Body'].read().decode('utf-8')
    filmmaker_data=json.loads(s3_data)
...
def distributor_parser(distributor_html_json_fp,run_date:str):
    s3_data=s3.get_object(Bucket=distributor_html_json_fp.split('/')[2],Key='/'.join(distributor_html_json_fp.split('/')[3:]))['Body'].read().decode('utf-8')
    distributors=json.loads(s3_data)
